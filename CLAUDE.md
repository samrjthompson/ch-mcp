...
## Rules
- Auth is Basic: key as username, empty password. Key comes from CH_API_KEY
  only. Never a literal, never logged, never in a fixture.
- Rate limit is 600 req / 5 min per key, shared across all endpoints. Every
  outbound call goes through a client.
